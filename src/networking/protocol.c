...
static uint64_t my_htonll(uint64_t val) {
    if (htonl(1) == 1) return val; // already big-endian
...
// Returns bytes received, -1 for error, 0 for nothing read (unknown packet type)
